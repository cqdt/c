...
public partial class Admin_Modules_ds_holy_japan : System.Web.UI.UserControl
{
    DataSet ds = new DataSet();
    public int total;
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!CMS.TTV.Rule.IsModuleRule("ds-holy-japan", CMS.TTV.Rules.APPROVE))
        {
            lblAlert.Text = "Bạn không có quyền thực hiện lệnh này";
            return;
        }
        total = CMS.MsSql.ExecuteToInt("select count(*) from T_HOLY");
    }
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        string str = "select ROW_NUMBER() OVER(ORDER BY PK_HOLY desc) as [TT], C_HOTEN [Họ và tên], CONVERT(varchar,C_NGAYSINH,103) [Ngày sinh], CONCAT('''',C_CMND) AS [Số CMND/căn cước công dân], C_GIOITINH [Giới tính], C_HKTT [Nơi đăng ký hộ khẩu thường trú (Tỉnh/thành phố)],"
                    ...
                    + " CONVERT(varchar,C_NGAYGUI,103) AS [Thời gian đăng ký trên hệ thống]"
                     + " FROM         T_HOLY ORDER BY PK_HoLy DESC";

        ds = CMS.MsSql.FillDataSet(str);
        Whatever.ExcelExport.ExportDataSetToExcel(ds, "danh-sach-dang-ky-du-tuyen-chuong-trinh-ho-ly-nhat-ban.xls");
    }
