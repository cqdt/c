...
public partial class Admin_Modules_eps : System.Web.UI.UserControl
{
    ...
    {
        if (!CMS.TTV.Rule.IsModuleRule("eps", CMS.TTV.Rules.APPROVE))
        {
            lblAlert.Text = "Bạn không có quyền thực hiện lệnh này";
            return;
        }
        string str = "SELECT * from T_TINHTRANG_HS";
    ...
    private void Inport(string sheet, int row, int BRow)
    {
        ...
        {
            TTV.Utils.Update_EPS(dt.Rows[i][0].ToString(), dt.Rows[i][1].ToString(), dt.Rows[i][2].ToString(), dt.Rows[i][3].ToString(), dt.Rows[i][4].ToString(), dt.Rows[i][5].ToString(), dt.Rows[i][6].ToString());            
        }
        string urladmin = "Default.aspx?ctl=eps&mess=Thao tác thành công";
