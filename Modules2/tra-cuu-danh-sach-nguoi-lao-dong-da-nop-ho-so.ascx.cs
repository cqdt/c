...
public partial class Modules_tra_cuu_danh_sach_nguoi_lao_dong_da_nop_ho_so : System.Web.UI.UserControl
{
    ...
    {
        Response.Redirect(Newwind.VQS.URL + "/tra-cuu-danh-sach-nguoi-lao-dong-da-nop-ho-so.aspx?n=" + txthoten.Text.Trim() + "&s=" + txtngaysinh.Text.Trim() + "");
    }

    public void Binddata(string hoten, string ngaysinh)
    {
        SqlCommand command = new SqlCommand("sp_NLD_danop_hoso");
        ...
        if (lRet != null && lRet.Tables.Count > 0)
        {
            ...
            this.dlKetqua.DataKeyField = "PK_HOSO";
